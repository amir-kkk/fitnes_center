...
def build_document():  # noqa: C901 – large intentionally
    doc = Document()
    fig_num = 1
    ...
    add_body(doc,
        ...
        "для фитнес-центра с трёхуровневой ролевой моделью (Клиент, Тренер, Администратор). "
        "Разрабатываемая система обеспечивает автоматизацию следующих процессов: управление "
        "каталогом абонементов, составление расписания групповых тренировок, онлайн-запись "
        "клиентов на занятия, система персональных тренировок с управлением слотами тренера, "
        "отслеживание персонального прогресса, AI-консультант, а также "
    ...
    add_body(doc,
        ...
        "которое предоставит клиентам возможность просматривать каталог абонементов, "
        "записываться на групповые и персональные тренировки, приобретать абонементы, "
        "отслеживать персональный прогресс и получать рекомендации AI-консультанта. "
        "Тренерам система должна предоставить инструменты управления расписанием персональных "
        "слотов и ведения показателей клиентов. Администраторам — полноценное управление "
        "всеми сущностями системы.")
    ...
    tasks = [
        ...
        "Спроектировать архитектуру приложения и базу данных с трёхуровневой ролевой моделью.",
        "Разработать серверную часть (REST API) с аутентификацией, авторизацией и валидацией.",
        "Реализовать систему персональных тренировок с управлением слотами и ценообразованием по рангу тренера.",
        "Реализовать AI-консультанта на базе DeepSeek API с дневным лимитом запросов и динамическим контекстом.",
    ...
    backend_techs = [
        ...
        ("OpenAI .NET SDK 2.1.0", "Официальный SDK для взаимодействия с OpenAI-совместимыми API. "
         "В проекте используется с кастомным Endpoint для работы через DeepSeek API (модель "
         "deepseek-chat). Обеспечивает конструирование истории диалога, отправку системного "
         "промпта и получение ответа модели через ChatClient.CompleteChatAsync."),
    ]
    ...
    add_body(doc,
        ...
        "визуализирующая взаимодействие актёров (Гость, Клиент, Тренер, Администратор) с системой. "
    ...
    add_body(doc,
        "В диаграмме выделены четыре актёра. Гость — анонимный пользователь, который может "
        "просматривать каталог абонементов, расписание и тренеров, а также регистрироваться "
        "и авторизоваться. Клиент наследует возможности Гостя и дополнительно может покупать "
        "абонементы, записываться на групповые и персональные тренировки, управлять прогрессом "
        "и использовать AI-консультанта (при наличии активного абонемента). Тренер имеет "
        "отдельный контур: управление персональными слотами, просмотр своего расписания, "
        "чтение и обновление показателей прогресса клиентов, с которыми есть активная запись. "
        "Администратор управляет всеми сущностями системы, включая назначение ролей, управление "
        "профилями тренеров и персональными записями клиентов.")
    ...
    add_body(doc,
        "Модель предметной области содержит 11 сущностей и 2 перечисления. Концептуальная "
        "ER-диаграмма (сущности и связи без технических атрибутов) представлена на рисунке ниже.")

    fig_num = add_screenshot_placeholder(doc,
        "Концептуальная ER-диаграмма (открыть docs/er-conceptual.drawio в draw.io и экспортировать как PNG)",
        fig_num)

    add_body(doc,
        "Физическая ER-диаграмма (таблицы с колонками и FK-связями) представлена на следующем рисунке.")

    fig_num = add_screenshot_placeholder(doc,
        "Физическая ER-диаграмма (открыть docs/er-physical.drawio в draw.io и экспортировать как PNG)",
        fig_num)

    add_body(doc,
        "Диаграмма классов (программная модель предметной области) представлена на рисунке ниже.")
    ...
    dev_items = [
        "Создание моделей данных (User, Membership, Training, PersonalWorkout, Purchase, Booking, Progress, AiTrainerMessage и др.).",
        "Разработка REST API с полным набором CRUD-операций для каждой сущности.",
        "Реализация трёхуровневой системы ролей (User, Trainer, Admin) с JWT-авторизацией и RBAC.",
        "Реализация системы персональных тренировок: управление слотами тренером, бронирование клиентом, ценообразование по формуле 1000 + (Ранг–1) × 500.",
        "Разработка AI-консультанта на базе DeepSeek API с динамическим системным промптом, дневным лимитом (10 сообщений/день) и проверкой активного абонемента.",
        ...
        "Реализация личного кабинета пользователя с отслеживанием прогресса, управлением записями и загрузкой аватара.",
        "Настройка Docker-контейнеризации для развёртывания одной командой.",
    ]
    ...
    add_body(doc,
        "Модель предметной области содержит 11 сущностей и 2 перечисления, "
    ...
    add_table_from_data(doc,
        ...
        [
            ...
            ["Email", "string (UQ)", "Адрес электронной почты (уникальный индекс)"],
            ["PasswordHash", "string", "Хеш пароля (BCrypt)"],
            ["Role", "string (max 20)", 'Роль: "Admin", "User" или "Trainer"'],
            ["FullName", "string", "Полное имя"],
            ["PhotoUrl", "string? (max 500)", "URL фото профиля (/uploads/users/... или /uploads/trainers/...)"],
            ["TrainerRank", "int? (1–5)", "Ранг тренера; NULL для ролей Admin и User"],
            ["CreatedAt", "DateTime", "Дата регистрации (UTC)"],
        ])
